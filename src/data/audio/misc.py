...
def preemphasis(signal, a=0):
    ...
    if torch.is_tensor(signal):
        assert signal.dim() == 1 or signal.dim() == 2
        return torch.cat((signal[:, 0:1], signal[:, 1:] - signal[:, :-1] * a), dim=-1)
...
def get_frames(signal, win_size, hop_size, win_fn=None):
    ...
    if torch.is_tensor(signal):
        ...
        # for numpy
        indices = (
            np.tile(np.arange(0, win_size), (numframes, 1))
            + np.tile(np.arange(0, numframes * hop_size, hop_size), (win_size, 1)).T
        )
...
def __get_inv_mel_filters_numpy(sr, n_fft, n_mels, fmin, fmax):
    return np.linalg.pinv(librosa.filters.mel(sr=sr, n_fft=n_fft, n_mels=n_mels, fmin=fmin, fmax=fmax)).astype(
        np.float32
    )
