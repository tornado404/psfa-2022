...
class TorchImpl:
    ...
    @staticmethod
    def inv_spectrogram(
        spec, sr, win_size, hop_size, win_fn="hamm", ref_db=20, top_db=100, normalize=False, n_iter=50, verbose=False
    ):
    ...
    def __griffin_lim(spectrogram, win_size, hop_size, win_fn, n_iter=20, verbose=False):
        ...
        def istft(stft):
            ...
            window_sum = window_sumsquare(
                "hann", N, n_fft=win_size, win_length=win_size, hop_length=hop_size, dtype=np.float32
            )
            approx_nonzero_indices = torch.from_numpy(np.where(window_sum > tiny(window_sum))[0]).to(stft.device)
...
def spectrogram(
    ...
    win_size, hop_size = _get_int_size(sr, win_size, hop_size)
    signal = misc.preemphasis(signal, preemphasis)
    return __backend.spectrogram(
        signal, sr, win_size, hop_size, win_fn, padding, ref_db, top_db, normalize, clip_normalized, subtract_mean
    )
...
def inv_spectrogram(
    ...
    win_size, hop_size = _get_int_size(sr, win_size, hop_size)
    inv = __backend.inv_spectrogram(spec, sr, win_size, hop_size, win_fn, ref_db, top_db, normalize, n_iter, verbose)
...
def inv_mel_spectrogram(
    ...
    inv = __backend.inv_mel_spectrogram(
        spec, sr, win_size, hop_size, win_fn, n_mels, fmin, fmax, ref_db, top_db, normalize, n_iter, verbose
    )
    return misc.deemphasis(inv, preemphasis)


def _get_int_size(sr, win_size, hop_size):
    assert type(win_size) == type(hop_size), f"win_size is {type(win_size)}, but hop_size is {type(hop_size)}"
    if isinstance(win_size, float):
        win_size = int(sr * win_size)
        hop_size = int(sr * hop_size)
    return win_size, hop_size
